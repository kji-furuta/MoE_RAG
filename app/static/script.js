...
function setupNavigation() {
    // 内部ナビゲーションリンクのみを処理
    const internalNavLinks = document.querySelectorAll('.nav-link:not(.external-link)');
    ...
    internalNavLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            const target = href.substring(1); // # を削除
            showSection(target);
        });
    });
    
    // 外部リンクの動作を確認（デバッグ用）
    const externalLinks = document.querySelectorAll('.nav-link.external-link');
    externalLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            console.log('External link clicked:', this.getAttribute('href'));
            // preventDefault()は呼ばず、デフォルトの動作を許可
        });
    });
}
