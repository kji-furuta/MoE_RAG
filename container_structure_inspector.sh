{
  `path`: `\\\\wsl$\\Ubuntu\\home\\kjifuruta\\AI_FT\\AI_FT_3\\container_structure_inspector.sh`,
  `content`: `#!/bin/bash
...
docker exec $CONTAINER_NAME bash -c \"ls -la /workspace/\" > \"$OUTPUT_DIR/workspace_root.txt\"
docker exec $CONTAINER_NAME bash -c \"find /workspace -maxdepth 2 -type d | sort\" > \"$OUTPUT_DIR/workspace_structure.txt\"
...
for dir in \"${IMPORTANT_DIRS[@]}\"; do
    ...
    echo \"   - $dir を調査中...\"
    docker exec $CONTAINER_NAME bash -c \"ls -la $dir 2>/dev/null || echo 'Directory not found'\" > \"$OUTPUT_DIR/dir_${dirname}.txt\"
done
...
docker inspect $CONTAINER_NAME --format='{{range .Mounts}}{{.Source}} -> {{.Destination}}{{println}}{{end}}' > \"$OUTPUT_DIR/volume_mounts.txt\"
...
# 9. レポートの生成
echo \"9. 調査レポートを生成中...\"
cat > \"$OUTPUT_DIR/inspection_report.md\" << EOF
# コンテナ内部構造調査レポート
...
\\`\\`\\`
$(cat \"$OUTPUT_DIR/volume_mounts.txt\")
\\`\\`\\`
