...
def assess_cot_quality(cot: str) -> Dict[str, any]:
    ...
    metrics = {
        ...
        'has_reasoning': any(keyword in cot for keyword in ['から', 'ため', 'により', 'したがって', 'よって', 'ので']),
        'word_count': len(cot),
        'score': 0.0
    }
...
def main(json_path: str):
    ...
    for idx, entry in enumerate(data, 1):
        ...
        if not is_valid:
            format_errors.extend([f"{entry_id}: {err}" for err in errors])
        ...
        if not cite_valid:
            citation_errors.extend([f"{entry_id}: {err}" for err in cite_errors])
    ...
    if cot_scores:
        ...
        for score in sorted(score_distribution.keys(), reverse=True):
            ...
            print(f"  {score:.1f}: {bar} ({count}件, {count/total_entries*100:.1f}%)")
    ...
    if valid_entries == total_entries and avg_cot_score >= 0.7:
        ...
        return 0
    elif valid_entries / total_entries >= 0.95 and avg_cot_score >= 0.6:
        print("⚠️ 条件付き合格: 一部エラーがありますが使用可能です")
        print(f"   - 有効率: {valid_entries/total_entries*100:.1f}% (基準: ≥95%)")
        print(f"   - CoT品質スコア: {avg_cot_score:.2f} (推奨: ≥0.70)")
        return 0
    else:
        print("❌ 検証失敗: データ修正が必要です")
        if valid_entries / total_entries < 0.95:
            print(f"   - 有効率が低すぎます: {valid_entries/total_entries*100:.1f}%")
        if avg_cot_score < 0.6:
            print(f"   - CoT品質が低すぎます: {avg_cot_score:.2f}")
        return 1

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("使用方法: python validate_rar_json.py <RARdata.json>")
        sys.exit(1)
